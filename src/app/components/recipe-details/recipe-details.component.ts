...
export class RecipeDetailsComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.route.params.subscribe(params => {
      ...
      if (idParam) {
        ...
        const idToCheck = isNumeric ? numericId : idParam;
        ...
        if (this.isLoggedIn) {
          this.checkFavorite(idToCheck);
        } else {
          this.checkGuestFavorite(idToCheck);
        }
      }
    });
  }
  ...
  checkGuestFavorite(recipeId: number | string): void {
    this.isFavorite = this.userDataService.checkIfGuestFavorite(recipeId);
  }
  ...
  toggleGuestFavorite(): void {
    if (!this.recipe) return;

    if (this.isFavorite) {
      this.userDataService.removeGuestFavorite(this.recipe.id);
      this.isFavorite = false;
    } else {
      const favorite: Favorite = {
